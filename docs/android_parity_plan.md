...
- 2026-05-21: Use-case layer step: added `domain/usecase` orchestration classes (`AuthUseCases`, `ClientUseCases`, `AdminUseCases`) and migrated `LoginViewModel`, `ClientHomeViewModel`, `AdminHomeViewModel` from direct repository calls to use-case calls.
- 2026-05-21: DI step: introduced application-level dependency container (`AtomGoApplication`, `di/AppContainer`) and switched `MainActivity` ViewModel creation to explicit factories (`LoginViewModelFactory`, `ClientHomeViewModelFactory`, `AdminHomeViewModelFactory`) with injected use-cases.
- 2026-05-21: UI decomposition step: extracted admin catalog formatting/status/date logic from `AdminCatalogDetails.kt` into dedicated `AdminCatalogFormatters.kt`, reducing screen file coupling and isolating non-UI rules.
- 2026-05-21: Test hardening step: updated unit tests for injected use-cases and stabilized admin login instrumentation flow with retry-based login helper (`performAdminLoginWithRetries`) for backend/network jitter.
- 2026-05-21: Verification step after use-case+DI migration: `:mobile:androidApp:compileDebugKotlin`, `:mobile:androidApp:testDebugUnitTest`, and `:mobile:androidApp:assembleDebug` are green.
- 2026-05-21: Emulator verification + screenshot report: focused instrumentation scenario `HomeNavigationUiTest#loginAsAdminClassic_opensAdminHome` passed on `emulator-5554`; fresh evidence captured at `docs/android_screenshots/2026-05-21-admin-home-after-usecase-di-step.png`.
