...
- корректировка долга из этой карточки должна работать по `ClientRentalRecord.id`, даже если аренда уже завершена;
- после корректировки пересчитываются `debtRub` / `totalAdjustmentRub` именно этой client_rental и сразу обновляется её `journalEntries`, потому что adjustment пишется в ledger с `rentalId = ClientRentalRecord.id`.

### Редактирование активной аренды (все поля)

`POST /api/v1/admin/rentals/{rentalId}` обновляет **активную lifecycle-аренду и её активную client_rental**.  
Поддерживаемые поля:
...
- `login/password` обновляются только парой (если передан один без второго -> `400`).
- Если lifecycle в `in_stock` и активной `ClientRentalRecord` нет, credentials обновить нельзя (`409`).
- При обновлении `password` проверяется глобальная уникальность `clientPasswordFingerprint` среди всех неудалённых `ClientRentalRecord` (кроме текущей редактируемой).
- При успешной смене credentials:
  - обновляется активная `ClientRentalRecord`;
  - обновляется `AppUser` этого клиента;
  - активные клиентские сессии этого клиента сбрасываются.
- `video_url/contract_url/comment` обновляются в lifecycle и в активной `ClientRentalRecord`, чтобы данные совпадали при открытии из обоих путей:
  - `Аренды -> карточка аренды`
  - `Клиенты -> карточка клиента -> история аренд`.
