...
- `ClientService`:
  ...
  - история клиентских аренд и клиентская задолженность
- `BikeService`:
  - CRUD велосипедов
  - вывод велосипеда из эксплуатации
- `RentService`:
  - создание `Аренды` как жизненного цикла велосипеда
  - смена статусов `long_term/soon_return/in_stock`
  - создание и закрытие `клиентских аренд`
  - удаление `Аренды` после вывода велосипеда из эксплуатации
...
- `DebtService`:
  - баланс клиентской аренды
  - клиентская задолженность после удаления аренды
  - корректировки
